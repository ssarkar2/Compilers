...
HeapMemoryManager::HeapMemoryManager(HeapMemoryManager &&other)
    : m_size(other.m_size), m_memory_buffer(other.m_memory_buffer) {
  other.m_memory_buffer = nullptr;
  other.m_size = 0;
}

HeapMemoryManager::HeapMemoryManager(const HeapMemoryManager &other)
    : m_size(other.m_size) {
  m_memory_buffer = new char[m_size];
